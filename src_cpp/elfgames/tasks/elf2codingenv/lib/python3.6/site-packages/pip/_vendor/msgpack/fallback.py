    ...
    def _read_header(self, execute=EX_CONSTRUCT):
        ...
        if b & 0b10000000 == 0:
            ...
            if self._max_ext_len < 1:
                raise UnpackValueError(
                    "%s exceeds max_ext_len(%s)" % (1, self._max_ext_len))
            self._reserve(2)
            ...
            if self._max_ext_len < 2:
                raise UnpackValueError(
                    "%s exceeds max_ext_len(%s)" % (2, self._max_ext_len))
            self._reserve(3)
            ...
            if self._max_ext_len < 4:
                raise UnpackValueError(
                    "%s exceeds max_ext_len(%s)" % (4, self._max_ext_len))
            self._reserve(5)
            ...
            if self._max_ext_len < 8:
                raise UnpackValueError(
                    "%s exceeds max_ext_len(%s)" % (8, self._max_ext_len))
            self._reserve(9)
            ...
            if self._max_ext_len < 16:
                raise UnpackValueError(
                    "%s exceeds max_ext_len(%s)" % (16, self._max_ext_len))
            self._reserve(17)
            n, obj = struct.unpack_from(
                "b16s", self._buffer_view, self._buff_i)
            self._buff_i += 17
        elif b == 0xd9:
            typ = TYPE_RAW
            self._reserve(1)
