...
def _class_escape(source, escape):
    ...
    try:
        ...
        if c == "x":
            ...
            if len(escape) != 4:
                raise source.error("incomplete escape %s" %
                                   escape, len(escape))
...
def _escape(source, escape, state):
    ...
    try:
        ...
        if c == "x":
            ...
            raise source.error("invalid group reference %d" %
                               group, len(escape) - 1)
        if len(escape) == 2:
            if c in ASCIILETTERS:
                raise source.error("bad escape %s" % escape, len(escape))
...
# pass # handle character classes
                        ...
                        if not first or subpattern:
                            ...
                            warnings.warn(
                                'Flags not at the start of the expression %r%s' % (
                                    # truncate long regexes
                                    source.string[:20],
                                    ' (truncated)' if len(
                                        source.string) > 20 else '',
                                ),
            ...
            if condgroup:
                p = _parse_sub_cond(
                    source, state, condgroup, verbose, nested + 1)
            else:
...
def parse_template(source, pattern):
    ...
    if not isinstance(source, str):
        ...
        literals = [None if s is None else s.encode(
            'latin-1') for s in literals]
