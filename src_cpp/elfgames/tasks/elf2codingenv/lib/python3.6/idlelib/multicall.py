...
class _ComplexBinder:
        ...
        if triplet[2] not in self.bindedfuncs:
            ...
            for s in _states:
                ...
                handler = self.__create_handler(lists, self.type,
                                                _state_codes[s])
                seq = "<%s%s-%s>" % (_state_names[s],
                                     self.typename, triplet[2])
                self.handlerids.append((seq, self.widget.bind(self.widgetinst,
                                                              seq, handler)))

        def doit(): return self.bindedfuncs[triplet[2]][triplet[0]].append(
            func)
    ...
    def unbind(self, triplet, func):
        def doit(): return self.bindedfuncs[triplet[2]][triplet[0]].remove(
            func)
        if not self.ishandlerrunning:
            doit()
        else:
