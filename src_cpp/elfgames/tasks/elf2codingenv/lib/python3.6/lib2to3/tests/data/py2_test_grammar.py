...
class GrammarTests(unittest.TestCase):
    ...
    def testFuncdef(self):
        ...
        if sys.platform.startswith('java'):
            self.assertEquals(v3.func_code.co_varnames,
                              ('a', '(b, c)', 'rest', 'b', 'c'))
        else:
            self.assertEquals(v3.func_code.co_varnames,
                              ('a', '.1', 'rest', 'b', 'c'))
    ...
    def testLambdef(self):
        ...
        def l3(): return [2 < x for x in [-1, 3, 0L]]
...
''')
    ...
    def testExec(self):
        ...
        warnings.filterwarnings(
            "ignore", "global statement", module="<string>")
    ...
    def testGenexps(self):
        ...
        self.assertEqual(sum(x for x in (y for y in (z for z in range(10)))), sum(
            [x for x in range(10)]))
        self.assertEqual(sum(x for x in [y for y in (z for z in range(10))]), sum(
            [x for x in range(10)]))
        self.assertEqual(sum(x for x in (y for y in (z for z in range(
            10) if True)) if True), sum([x for x in range(10)]))
        self.assertEqual(
    ...
    def testComprehensionSpecials(self):
        ...
        x = 5
        t = True
        self.assertEqual([(i, j) for i in range(10)
                          for j in range(5)], list(g))

        # Grammar allows multiple adjacent 'if's in listcomps and genexps,
        # even though it's silly. Make sure it works (ifelse broke this.)
        self.assertEqual([x for x in range(10) if x % 2 if x % 3], [1, 5, 7])
        self.assertEqual(list(x for x in range(10) if x %
                              2 if x % 3), [1, 5, 7])
    ...
    def testIfElseExpr(self):
        ...
        self.assertEqual([x(False) for x in (
            lambda x: False if x else True, lambda x: True if x else False) if x(False)], [True])
        ...
        self.assertEqual((1 or _checkeval("check 4", 2)
                          if 1 else _checkeval("check 5", 3)), 1)
