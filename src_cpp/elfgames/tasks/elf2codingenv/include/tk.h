...
typedef int(Tk_OptionParseProc)(
    ...
    char* widgRec,
    int offset);
typedef CONST86 char*(Tk_OptionPrintProc)(
    ClientData clientData,
    Tk_Window tkwin,
    char* widgRec,
    int offset,
...
#define Tk_GetClassProc(procs, which)                                      \
  (((procs) == NULL) ? NULL                                                \
                     : (((procs)->size <= Tk_Offset(Tk_ClassProcs, which)) \
                            ? NULL                                         \
                            : (procs)->which))
...
typedef void(
    Tk_ItemDeleteProc)(Tk_Canvas canvas, Tk_Item* itemPtr, Display* display);
typedef void(Tk_ItemDisplayProc)(
    ...
    int height);
typedef double(
    Tk_ItemPointProc)(Tk_Canvas canvas, Tk_Item* itemPtr, double* pointPtr);
typedef int(
    Tk_ItemAreaProc)(Tk_Canvas canvas, Tk_Item* itemPtr, double* rectPtr);
...
#endif /* USE_OLD_CANVAS */
typedef void(
    Tk_ItemDCharsProc)(Tk_Canvas canvas, Tk_Item* itemPtr, int first, int last);
...
typedef struct Tk_ItemType {
   ...
   * type. */
  CONST86 Tk_ConfigSpec*
      configSpecs; /* Pointer to array of configuration specs for
...
#define Tk_Main(argc, argv, proc) \
  Tk_MainEx(argc, argv, proc, (Tcl_FindExecutable(0), (Tcl_CreateInterp)()))
#else
#define Tk_Main(argc, argv, proc) \
  Tk_MainEx(                      \
      argc, argv, proc, (Tcl_FindExecutable(argv[0]), (Tcl_CreateInterp)()))
#endif
const char* Tk_InitStubs(Tcl_Interp* interp, const char* version, int exact);
EXTERN const char*
Tk_PkgInitStubsCheck(Tcl_Interp* interp, const char* version, int exact);
