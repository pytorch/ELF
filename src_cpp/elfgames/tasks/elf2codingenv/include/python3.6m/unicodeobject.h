...
#define PyUnicode_READY(op)     \
  (assert(PyUnicode_Check(op)), \
   (PyUnicode_IS_READY(op) ? 0 : _PyUnicode_Ready((PyObject*)(op))))
...
PyAPI_FUNC(PyObject*) _PyUnicode_FromASCII(const char* buffer, Py_ssize_t size);
#endif

#if !defined(Py_LIMITED_API) || Py_LIMITED_API + 0 >= 0x03030000
PyAPI_FUNC(PyObject*)
    PyUnicode_Substring(PyObject* str, Py_ssize_t start, Py_ssize_t end);
#endif
...
PyAPI_FUNC(Py_UCS4) PyUnicode_ReadChar(PyObject* unicode, Py_ssize_t index);
...
PyAPI_FUNC(int)
    PyUnicode_WriteChar(PyObject* unicode, Py_ssize_t index, Py_UCS4 character);
#endif
...
#define _PyUnicodeWriter_Prepare(WRITER, LENGTH, MAXCHAR)         \
  (((MAXCHAR) <= (WRITER)->maxchar &&                             \
    (LENGTH) <= (WRITER)->size - (WRITER)->pos)                   \
       ? 0                                                        \
       : (((LENGTH) == 0) ? 0 : _PyUnicodeWriter_PrepareInternal( \
...
#define _PyUnicodeWriter_PrepareKind(WRITER, KIND)                      \
  (assert((KIND) != PyUnicode_WCHAR_KIND),                              \
   (KIND) <= (WRITER)->kind ? 0 : _PyUnicodeWriter_PrepareKindInternal( \
                                      (WRITER), (KIND)))
...
#ifndef Py_LIMITED_API
PyAPI_FUNC(char*) PyUnicode_AsUTF8AndSize(PyObject* unicode, Py_ssize_t* size);
...
PyAPI_FUNC(PyObject*) PyUnicode_EncodeFSDefault(PyObject* unicode);
...
PyAPI_FUNC(Py_UNICODE*) Py_UNICODE_strcat(Py_UNICODE* s1, const Py_UNICODE* s2);
...
PyAPI_FUNC(Py_UNICODE*) Py_UNICODE_strrchr(const Py_UNICODE* s, Py_UNICODE c);
...
PyAPI_FUNC(Py_UNICODE*) PyUnicode_AsUnicodeCopy(PyObject* unicode);
#endif /* Py_LIMITED_API */
