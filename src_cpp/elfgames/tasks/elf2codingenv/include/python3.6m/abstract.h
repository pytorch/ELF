...
#ifndef Py_LIMITED_API
PyAPI_FUNC(PyObject*) _PyStack_AsTuple(PyObject** stack, Py_ssize_t nargs);

/* Convert keyword arguments from the (stack, kwnames) format to a Python
   dictionary.

   kwnames must only contains str strings, no subclass, and all keys must
   be unique. kwnames is not checked, usually these checks are done before or
   later
   calling _PyStack_AsDict(). For example, _PyArg_ParseStack() raises an
   error if a key is not a string. */
PyAPI_FUNC(PyObject*) _PyStack_AsDict(PyObject** values, PyObject* kwnames);
...
PyAPI_FUNC(PyObject*)
    PyObject_CallFunction(PyObject* callable_object, const char* format, ...);
...
PyAPI_FUNC(PyObject*) _PyObject_CallMethod_SizeT(
    PyObject* o,
    const char* name,
    const char* format,
    ...);
#ifndef Py_LIMITED_API
PyAPI_FUNC(PyObject*) _PyObject_CallMethodId_SizeT(
    PyObject* o,
    _Py_Identifier* name,
    const char* format,
    ...);
...
PyAPI_FUNC(PyObject*)
    PyObject_CallMethodObjArgs(PyObject* o, PyObject* method, ...);
#ifndef Py_LIMITED_API
...
PyAPI_FUNC(int) PyObject_AsReadBuffer(
    PyObject* obj,
    const void** buffer,
    Py_ssize_t* buffer_len);
...
PyAPI_FUNC(int) PyObject_AsWriteBuffer(
    PyObject* obj,
    void** buffer,
    Py_ssize_t* buffer_len);
...
PyAPI_FUNC(int) PyBuffer_ToContiguous(
    void* buf,
    Py_buffer* view,
    Py_ssize_t len,
    char order);
...
#if !defined(Py_LIMITED_API) || Py_LIMITED_API + 0 >= 0x03050000
PyAPI_FUNC(PyObject*)
    PyNumber_InPlaceMatrixMultiply(PyObject* o1, PyObject* o2);
...
PyAPI_FUNC(PyObject*)
    PySequence_GetSlice(PyObject* o, Py_ssize_t i1, Py_ssize_t i2);
...
PyAPI_FUNC(int)
    PySequence_SetSlice(PyObject* o, Py_ssize_t i1, Py_ssize_t i2, PyObject* v);
