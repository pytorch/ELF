...
/* This structure helps managing static strings. The basic usage goes like this:
   ...
   usage, the string "foo" is interned, and the structures are linked. On
   interpreter
...
#define _Py_static_string(varname, value) \
  static _Py_Identifier varname = _Py_static_string_init(value)
...
typedef int (
    *ssizessizeobjargproc)(PyObject*, Py_ssize_t, Py_ssize_t, PyObject*);
...
PyAPI_FUNC(int) _PyObject_GenericSetAttrWithDict(
    PyObject*,
    PyObject*,
    PyObject*,
    PyObject*);
...
#define _Py_NewReference(op)                                                  \
  (_Py_INC_TPALLOCS(op)                                                       \
       _Py_COUNT_ALLOCS_COMMA _Py_INC_REFTOTAL _Py_REF_DEBUG_COMMA Py_REFCNT( \
           op) = 1)
...
#define _Py_Dealloc(op) \
  (_Py_INC_TPFREES(op)  \
       _Py_COUNT_ALLOCS_COMMA(*Py_TYPE(op)->tp_dealloc)((PyObject*)(op)))
#endif
#endif /* !Py_TRACE_REFS */

#define Py_INCREF(op) \
  (_Py_INC_REFTOTAL _Py_REF_DEBUG_COMMA((PyObject*)(op))->ob_refcnt++)

#define Py_DECREF(op)                                                        \
  do {                                                                       \
    PyObject* _py_decref_tmp = (PyObject*)(op);                              \
    if (_Py_DEC_REFTOTAL _Py_REF_DEBUG_COMMA--(_py_decref_tmp)->ob_refcnt != \
        0)                                                                   \
      _Py_CHECK_REFCNT(_py_decref_tmp) else _Py_Dealloc(_py_decref_tmp);     \
  } while (0)
...
#define Py_TRASHCAN_SAFE_END(op)                                         \
  ...
  else _PyTrash_thread_deposit_object((PyObject*)op);                    \
  }                                                                      \
  while (0)                                                              \
    ;

#ifndef Py_LIMITED_API
PyAPI_FUNC(void) _PyDebugAllocatorStats(
    FILE* out,
    const char* block_name,
    int num_blocks,
    size_t sizeof_block);
PyAPI_FUNC(void) _PyObject_DebugTypeStats(FILE* out);
#endif /* ifndef Py_LIMITED_API */
