...
#define TK_FW_UNKNOWN                               \
  -1 /* Unknown weight. This value is used for      \
...
#define TK_FS_OBLIQUE                              \
  2 /* This value is only used when parsing X font \
     * names to determine the closest match. It is \
     * only stored in the XLFDAttributes           \
     * structure, never in the slant field of the  \
     * TkFontAttributes. */

#define TK_FS_UNKNOWN                               \
  -1 /* Unknown slant. This value is used for error \
...
#define TK_SW_UNKNOWN                             \
  3 /* Unknown setwidth. This value may be stored \
...
MODULE_SCOPE int TkFontParseXLFD(
    const char* string,
...
MODULE_SCOPE int TkCreateNamedFont(
    Tcl_Interp* interp,
    Tk_Window tkwin,
    const char* name,
    TkFontAttributes* faPtr);
MODULE_SCOPE int
TkDeleteNamedFont(Tcl_Interp* interp, Tk_Window tkwin, const char* name);
MODULE_SCOPE int
TkFontGetFirstTextLayout(Tk_TextLayout layout, Tk_Font* font, char* dst);
...
MODULE_SCOPE TkFont* TkpGetFontFromAttributes(
    TkFont* tkFontPtr,
    Tk_Window tkwin,
    const TkFontAttributes* faPtr);
MODULE_SCOPE void TkpGetFontFamilies(Tcl_Interp* interp, Tk_Window tkwin);
