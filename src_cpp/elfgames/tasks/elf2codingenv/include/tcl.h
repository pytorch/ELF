...
#define TCL_THREAD_NOFLAGS          \
  (0000) /* Standard flags, default \
          * behaviour. */
...
#define TCL_REG_EXPANDED                   \
  000040 /* Expanded format, white space & \
          * comments. */
#define TCL_REG_NLSTOP 000100 /* \n doesn't match . or [^ ] */
#define TCL_REG_NLANCH 000200 /* ^ matches after \n, $ before. */
#define TCL_REG_NEWLINE 000300 /* Newlines are line terminators. */
#define TCL_REG_CANMATCH                      \
  001000 /* Report details on partial/limited \
          * matches. */
...
typedef void(
    Tcl_DupInternalRepProc)(struct Tcl_Obj* srcPtr, struct Tcl_Obj* dupPtr);
...
#define TCL_ENSEMBLE_PREFIX                      \
  0x02 /* Flag value to say whether to allow     \
...
typedef Tcl_HashEntry*(
    Tcl_AllocHashEntryProc)(Tcl_HashTable* tablePtr, void* keyPtr);
...
struct Tcl_HashTable {
  ...
  Tcl_HashEntry* (
      *createProc)(Tcl_HashTable* tablePtr, const char* key, int* newPtr);
 ...
 */
typedef int(
    Tcl_DriverTruncateProc)(ClientData instanceData, Tcl_WideInt length);
...
typedef int(Tcl_FSFileAttrsGetProc)(
    ...
    Tcl_Obj* pathPtr,
    Tcl_Obj** objPtrRef);
typedef const char* CONST86*(
    Tcl_FSFileAttrStringsProc)(Tcl_Obj* pathPtr, Tcl_Obj** objPtrRef);
typedef int(Tcl_FSFileAttrsSetProc)(
    ...
    Tcl_Obj* objPtr);
typedef Tcl_Obj*(
    Tcl_FSLinkProc)(Tcl_Obj* pathPtr, Tcl_Obj* toPtr, int linkType);
...
typedef int(
    Tcl_FSPathInFilesystemProc)(Tcl_Obj* pathPtr, ClientData* clientDataPtr);
...
typedef int(
    Tcl_ArgvFuncProc)(ClientData clientData, Tcl_Obj* objPtr, void* dstPtr);
...
#define TCL_ARGV_AUTO_REST \
  { TCL_ARGV_REST, "--", NULL, NULL, "Marks the end of the options", NULL }
...
EXTERN void Tcl_MainEx(
    int argc,
    char** argv,
    Tcl_AppInitProc* appInitProc,
    Tcl_Interp* interp);
EXTERN const char*
Tcl_PkgInitStubsCheck(Tcl_Interp* interp, const char* version, int exact);
...
#define ckalloc(x) ((void*)Tcl_DbCkalloc((unsigned)(x), __FILE__, __LINE__))
...
#define Tcl_NewBignumObj(val) Tcl_DbNewBignumObj(val, __FILE__, __LINE__)
...
#define Tcl_NewWideIntObj(val) Tcl_DbNewWideIntObj(val, __FILE__, __LINE__)
#endif /* TCL_MEM_DEBUG */
