...
class GameContext {
 public:
  using ThreadedDispatcher = ChouFleurGameSelfPlay::ThreadedDispatcher;
  ...
  GameContext(const ContextOptions& contextOptions, const GameOptions& options)
    ...
    std::cout << " C++ GameContext" << std::endl;
    ...
    if (options.mode == "train" || options.mode == "offline_train") {
      ...
      for (int i = 0; i < numGames; ++i) {
        games_.emplace_back(new ChouFleurTrainGame(
      ...
      for (int i = 0; i < numGames; ++i) {
        games_.emplace_back(new ChouFleurGameSelfPlay(
            i,
            gc,
            contextOptions,
            options,
  ...
  const ChouFleurGameBase* getGame(int game_idx) const {
 ...
 private:
  ...
  std::vector<std::unique_ptr<ChouFleurGameBase>> games_;

  std::unique_ptr<Server> server_;
  std::unique_ptr<Client> client_;

  ChouFleurFeature goFeature_;
