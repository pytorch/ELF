...
ChouFleurTrainGame::ChouFleurTrainGame(
    int game_idx,
    elf::GameClient* client,
    const ContextOptions& context_options,
    const GameOptions& options,
    elf::shared::ReaderQueuesT<Record>* reader)
    : ChouFleurGameBase(game_idx, client, context_options, options), reader_(reader) {
  for (size_t i = 0; i < kNumState; ++i) {
    _state_ext.emplace_back(new ChouFleurStateExtOffline(game_idx, options));
  }
}

void ChouFleurTrainGame::act() {
  ...
  for (size_t i = 0; i < kNumState; ++i) {
    ...
    //_state_ext[i]->generateD4Code(&_rng);   FIXME presumably in the general case of a possibly non-rotable game I can remove this.
