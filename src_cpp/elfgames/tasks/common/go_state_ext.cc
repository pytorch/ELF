...
/*std::string ChouFleurStateExt::dumpSgf(const std::string& filename) const {
...
void ChouFleurStateExt::showFinishInfo(FinishReason reason) const {
  ...
  switch (reason) {
    case FR_RESIGN:
      std::cout << "Player " << std::to_string(player) << " resigned at "
                << _state.getPly()
