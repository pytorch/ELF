...
struct ChouFleurStateExt {
 public:
  ChouFleurStateExt(int game_idx, const GameOptions& options)
      : _game_idx(game_idx),
...
//  void dumpSgf() const {
  ...
  return std::string("dumpSgf not implemented in ChouFleur");
  }
 // void dumpSgf() const {}
  ...
  float setFinalValue(FinishReason reason, std::mt19937* rng) {
    UNUSED(reason);
    UNUSED(rng);
    if (_state.GetStatus() == 3) { return -1.; };   // -1 means that black wins.  FIXME
    if (_state.GetStatus() == 4) { return 1.; };    // 1 means that white wins.
    return 0.; /*
  ...
  Record dumpRecord() const {
    ...
    if (_state.GetStatus() == 3) {
      r.result.content = "black wins";
      r.result.reward = -1;
    } else {
      if (_state.GetStatus() == 4) {
        r.result.content = "white wins";
        r.result.reward = 1;
      } else {
        if (_state.GetStatus() == 2) {
        r.result.content = "draw";
        r.result.reward = 0;
        } else {
          r.result.content = "in progress";
          r.result.reward = 0;
        }
      }
    }
    //r.result.content = _state.GetStatus() == 3 ? "black wins" : "white wins"; //coords2sgfstr(_state.getAllMoves());
    ...
    r.result.num_move = -1; //_state.getPly() - 1;
  ...
  void addMCTSPolicy(
    ...
    std::fill(
        ...
        _mcts_policies.back().prob + StateForChouFleurNumActions,
        0);
    for (size_t k = 0; k < policy.size(); k++) {
      const auto& entry = policy[k];
      unsigned char c =
...
class ChouFleurStateExtOffline {
 public:
  friend class ChouFleurFeature;

  ChouFleurStateExtOffline(int game_idx, const GameOptions& options)
 ...
 private:
  const int _game_idx;
// public: // FIXME this is public just because I am deadly tired right now...
  StateForChouFleur _state;
  BoardFeature _bf;
 protected:
  GameOptions _options;
