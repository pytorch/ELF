...
////////////////// GoGame /////////////////////
ChouFleurGameSelfPlay::ChouFleurGameSelfPlay(
    int game_idx,
    ...
    : ChouFleurGameBase(game_idx, client, context_options, options),
...
MCTSChouFleurAI* ChouFleurGameSelfPlay::init_ai(
  ...
  return new MCTSChouFleurAI(opt, [&](int) { return new MCTSActor(client_, params); });
}

Coord ChouFleurGameSelfPlay::mcts_make_diverse_move(MCTSChouFleurAI* mcts_go_ai, Coord c) {
  auto policy = mcts_go_ai->getMCTSPolicy();

    /*    FIXME I am not sure of what I am doing here... can I just skip this ?
  bool diverse_policy =
...
Coord ChouFleurGameSelfPlay::mcts_update_info(MCTSChouFleurAI* mcts_go_ai, Coord c) {
...
/*  bool we_are_good = _state_ext.state().nextPlayer() == 1 //S_BLACK   FIXME S_BLACK==1
...
void ChouFleurGameSelfPlay::restart() {
...
void ChouFleurGameSelfPlay::act() {
  if (_online_counter % 5 == 0) {
    using std::placeholders::_1;
    using std::placeholders::_2;
    auto f = std::bind(&ChouFleurGameSelfPlay::OnReceive, this, _1, _2);
  ...
  if (_human_player != nullptr) {
    do {
      ...
      if (reply.c == M_SKIP)
        break;
      if (reply.c == M_CLEAR) {
        if (!_state_ext.state().justStarted()) {  //FIXME: presumably I can just skip this.
...
//          Y(reply.c),
  ...
  Coord c = M_INVALID;
  bool use_dga = 
      (player == S_WHITE && _options.white_use_dga) ||
